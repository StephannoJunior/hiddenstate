...
## Putting it on hiddenstategroup.com

Easiest path, no server to manage:

1. Put this folder on [GitHub](https://github.com) as a repository.
2. Sign up at [Vercel](https://vercel.com) (or [Netlify](https://netlify.com)) and
   connect that repository. It auto-detects Vite and builds it — no config needed.
3. In the host's dashboard, add your domain `hiddenstategroup.com`. It'll give
   you DNS records to enter wherever you bought the domain.
4. DNS takes anywhere from a few minutes to a few hours to take effect. HTTPS is
   automatic on both hosts.

After that, every push to GitHub redeploys the site automatically.

**Deep links are already handled.** `vercel.json` and `public/_redirects` are
both included, which is what stops `hiddenstategroup.com/records` from showing
a 404 when someone opens it directly or refreshes the page. Don't delete them.
